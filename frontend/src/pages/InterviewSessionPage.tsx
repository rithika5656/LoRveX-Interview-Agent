...
import type { InterviewSessionState } from "../types/interview";
...
export function InterviewSessionPage() {
  ...
  const { config } = sessionState;
  const roleLabel = formatRoleLabel(config.targetRole, config.customRole);
  const resumeLabel = config.resume ? `${config.resume.name} • Ready` : "Not provided";
      ...
      <section className="mx-auto max-w-4xl rounded-[2rem] border border-slate-200 bg-white p-8 shadow-[0_20px_60px_-34px_rgba(15,23,42,0.28)] sm:p-10">
        ...
        <div className="mt-8 grid gap-4 rounded-[1.75rem] border border-slate-200 bg-slate-50 p-6 sm:grid-cols-2">
          <div>
            ...
            <p className="mt-2 text-lg font-semibold text-slate-950">{config.candidateName}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mt-2 text-lg font-semibold text-slate-950">{config.interviewType}</p>
          </div>
          <div>
            <p className="text-sm uppercase tracking-[0.28em] text-slate-500">Difficulty</p>
            <p className="mt-2 text-lg font-semibold text-slate-950">{config.difficulty}</p>
          </div>
          <div>
            <p className="text-sm uppercase tracking-[0.28em] text-slate-500">Duration</p>
            <p className="mt-2 text-lg font-semibold text-slate-950">{config.duration} minutes</p>
          </div>
          <div>
